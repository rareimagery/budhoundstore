...
const BASE = import.meta.env.VITE_DRUPAL_URL || "";
...
// ── Request: attach OAuth + cart + store tokens ──────────────────────────────
drupalClient.interceptors.request.use((config) => {
  ...
  const storeId = localStorage.getItem("budstore_current_store_id");
  if (storeId) config.headers["Commerce-Current-Store"] = storeId;
